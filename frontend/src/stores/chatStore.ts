...
export const useChatStore = create<ChatStore>((set) => ({
  ...
  createSession: async (modelSettings: ModelSettings, systemPrompt?: string) => {
    console.log('[ChatStore] 开始创建新会话，模型配置:', modelSettings);
    console.log('[ChatStore] System Prompt:', systemPrompt);
    ...
    try {
      ...
      console.log('[ChatStore] 准备发送的数据:', requestData);

      const response = await axios.post('/api/chat/sessions', requestData);
      console.log('[ChatStore] 创建会话成功，服务器响应:', response.data);
      ...
      console.log('[ChatStore] 格式化后的新会话数据:', newSession);
